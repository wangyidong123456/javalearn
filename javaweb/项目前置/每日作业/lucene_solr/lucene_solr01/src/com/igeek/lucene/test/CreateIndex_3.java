...
public class CreateIndex_3 {
	...
	public static void main(String[] args) throws Exception {
		...
		for (Map<String, Object> map : list) {
			...
			// 图书ID
			doc.add(new TextField("id", map.get("item_id").toString(), Store.YES));
			// 图书名称
			doc.add(new TextField("name", map.get("item_title").toString(), Store.YES));
			// 图书价格
			doc.add(new TextField("price", map.get("item_price").toString(), Store.YES));
			// 图书图片
			doc.add(new TextField("pic", map.get("item_pic").toString(), Store.YES));
			// 图书描述
